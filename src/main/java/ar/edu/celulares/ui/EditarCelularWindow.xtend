...
class EditarCelularWindow extends Dialog<Celular> {
	...
	def homeCelulares() {
		ApplicationContext.instance.getSingleton(typeof(HomeCelulares)) as HomeCelulares
	}

	def getHomeModelos() {
		ApplicationContext.instance.getSingleton(typeof(HomeModelos)) as HomeModelos
	}
